...
def generate_unique_id_with_check():
        ...
        while tech_event_register.objects.filter(id=unique_id).exists():
            unique_id = generate_unique_id()
        return unique_id


class tech_event_register(models.Model):
    ...
    event = models.ForeignKey("tech_event", verbose_name=("Event Name"), on_delete=models.SET_NULL, null=True)
