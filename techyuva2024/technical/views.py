...
def all_technical_events(request):
    allevents = tech_event.objects.all()
    context = {
        "allevents":allevents
        }
    return render(request,"eventpage/technical.html",context)


def event_detail_page(request,pk):
    event_detail = tech_event.objects.filter(slug = pk)
    context = {
        "event_detail":event_detail
    }
    return render(request,'eventpage/event_details.html',context)

def event_register_page(request,pk):
    event_detail = tech_event.objects.filter(slug = pk)
    event_detail_instance = tech_event.objects.get(slug = pk)
    ...
    if request.method == "POST" and request.FILES.get('paymentproof'):
        ...
        new_instance = tech_event_register(name=name, email=email, ph=phone,collegename=collegename,usn=collegeid,event=event_detail_instance)
        new_instance.paymentproof.save(f'{name}_{email}.png', paymentproofsf, save=True)
        url = "http://techyuva.nexel.in/events/Technical-Events/proof/"+new_instance.slug
        ...
        return redirect('proof', new_instance.slug)
...
def proof(request,pk):
    register = tech_event_register.objects.filter(slug=pk)
    context = {
        'register':register
    }
